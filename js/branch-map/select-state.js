...
(function(plugin){
  ...
  docReady(function(){
    ...
    map.hooks.click_state = function(state, e){   
      if (me.on_shift){ //select on shift+click
        var evt = e || w.event;
        var length=window[plugin].selected.length;
        var index=window[plugin].selected.indexOf(state);  
        var last_state=window[plugin].selected[length-1];
        if (length == 0){
            window[plugin].select(state);
        }        
        else if (length > 0){
          if(evt.shiftKey){
            if (index > -1){
              window[plugin].deselect(state);
            }
            else{
              window[plugin].select(state);
            }
            ...
            window[plugin].deselect_all(last_state);
            window[plugin].select(state);
          }
        }        
      }
